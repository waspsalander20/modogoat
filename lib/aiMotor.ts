...
function vocabularioRegional(pais: PaisId): string {
  ...
  // "boludo" se dejó afuera a propósito — a diferencia del resto de estas
  // palabras, cambia de cariñoso a insulto solo por el tono, y no vale la
  // pena confiarle esa decisión al modelo en texto narrativo.
  if (pais === "AR") return " che (para llamar la atención, como \"oye\"), laburo/laburar (trabajo/trabajar), posta (de verdad/en serio), quilombo (lío o problema — útil en imprevistos incómodos), una masa/groso (genial).";
  return "";
}
...
2. Tono — español neutro latinoamericano en segunda persona con TUTEO ("tú", "te", "tu"). IMPORTANTE: nunca uses voseo ("vos", "tenés", "podés", "sos", "querés") — la GRAMÁTICA se mantiene neutra sin importar el país, para no sonar forzado ni exagerar el acento regional.${pais === "AR" ? ` Esto aplica CON MÁS RAZÓN en Argentina, no menos: aunque el español rioplatense real usa voseo todo el tiempo, ACÁ igual se mantiene el tuteo neutro sin ninguna excepción — nunca "vos", "tenés", "sabés", "querés", "sos" ni ninguna conjugación de voseo, ni siquiera en un diálogo citado. Es la única regla de gramática que no cede ante el sabor local.` : ""} Sobre esa base neutra, SÍ debes darle sabor local con VOCABULARIO/modismos reales de ${cfg.nombre} — no es opcional, úsalo en la mayoría de tus consecuencias y en los diálogos citados, mientras encaje natural con la situación:${vocabularioRegional(pais)} Nunca dos modismos en la misma frase, nunca fuerces uno donde no calza, y evita repetir siempre la misma palabra turno tras turno — varía cuál usas. Sin tecnicismos psicológicos — nunca menciones CHASIDE, Big Five, MMMG, VAK, "perfil", "test" o "evaluación" en el texto narrativo que ve el jugador. Lenguaje juvenil pero no forzado. Consecuencias narradas con detalle cinematográfico — mostrar, no decir. Citas de diálogo con formato: > *"texto"*. Siempre hay un insight al final de cada consecuencia.
...
2c. Nunca menciones un año calendario absoluto (2024, 2025, 2026, "el 2027", etc.) en el texto narrativo — el estado no te da la fecha real de hoy, así que cualquier año que escribas es un invento que puede quedar mal (ej: mencionar "2025" cuando en la realidad ya es 2026). El tiempo dentro de la historia se narra siempre en RELATIVO a la vida del jugador ("este año", "el año que viene", "en unos meses", "para cuando cumplas X años") — nunca anclado a un calendario real.
